...
#' @importClassesFrom h2o H2OFrame
pipe <- function()
{
  function(lhs, rhs)
  {
    ...
    env[["_function_list"]] <-
      lapply(seq_along(rhss), function(i)
        magrittr:::wrap_function(rhss[[i]], pipes[[i]], parent))
    ...
    if (magrittr:::is_placeholder(lhs)) {
      ...
      # check out if the lhs is a H2OFrame
      if_lhs_h2o <- eval(parse(
        text = paste0("is(", deparse(lhs), ", 'H2OFrame')")), parent, parent)

      # check if the first function to be applied is exported by h2o package
      rhs_can_handle <-
        as.character(rhss[[1]][[1]]) %in% getNamespaceExports("h2o")
      lhs_to_eval <- lhs

      # if the lhs is an H2OFrame, and the next function can not handle it, then
      # convert the H2OFrame to data.frame
      if (if_lhs_h2o && !rhs_can_handle) {
        lhs_to_eval <-
          parse(text = paste0("as.data.frame(", deparse(lhs), ")"))
      }

      # evaluate the LHS
      env[["_lhs"]] <- eval(lhs_to_eval, parent, parent)

      # compute the result by applying the function to the LHS
      result <- withVisible(eval(quote(`_fseq`(`_lhs`)), env, env))

      # if the result is a matrix/data.frame, and the input was a H2OFrame,
      # then convert the result to an H2OFrame
      if (!is.null(dim(result$value)) && if_lhs_h2o && !rhs_can_handle) {
        result$value <- as.h2o(result$value)
      }
